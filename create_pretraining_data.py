...
flags.DEFINE_string("vocab_file", None, "The vocabulary file that the ALBERT model was trained on.")

flags.DEFINE_string("input_file_mode", "rb", "The data format of the input file.")

flags.DEFINE_string("tokenizer_type", None, "Tokenizer type (character, wordpiece).")
...
flags.DEFINE_integer("dupe_factor", 40, "Number of times to duplicate the input data (with different masks).")
...
def write_instance_to_example_files(instances, tokenizer, max_seq_length, max_predictions_per_seq, output_files):
    ...
    for output_file in output_files:
        writers.append(tf.python_io.TFRecordWriter(output_file, options="GZIP"))
...
def create_training_instances(input_files, tokenizer, max_seq_length,
    ...
    print(f'NUM DOCUMENTS: {len(all_documents)}')
...
def create_masked_lm_predictions(tokens, masked_lm_prob, max_predictions_per_seq, vocab_words, rng):
    ...
    for (i, token) in enumerate(tokens):
        ...
        if (FLAGS.do_whole_word_mask and len(cand_indexes) >= 1 and not is_start_piece(token)):
...
def main(_):
    ...
    tokenizer = tokenization.BertJapaneseTinySegmenterTokenizer(
        FLAGS.vocab_file,
